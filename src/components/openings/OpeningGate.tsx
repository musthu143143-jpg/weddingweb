...
function TanjoreOpening({ t, template, data, onComplete }: { t: TemplateTheme; template: WeddingTemplate; data?: InvitationData; onComplete: () => void }) {
  ...
  useEffect(() => {
    if (!opened) return;
    // Let the doors finish their full, unhurried reveal before the gate fades.
    const id = window.setTimeout(onComplete, reduce ? 260 : 3450);
    return () => window.clearTimeout(id);
  ...
  return (
    ...
    >
      ...
      <div
        className="relative z-10 -translate-y-5 aspect-[9/16] max-w-[calc(100vw-1rem)] sm:translate-y-0"
        style={{ width: "min(92vw, 680px, calc((100svh - 7.5rem) * 0.5625))", transformStyle: "preserve-3d" }}
      >
        <div className="absolute inset-0 overflow-hidden" style={{ background: "#2A0B0F", boxShadow: "0 30px 80px #090204aa" }}>
          <TanjoreInnerCard t={t} data={data} />

          <motion.div
            className="absolute inset-y-0 left-0 z-20 w-1/2 origin-left"
            animate={{ rotateY: opened ? -108 : 0 }}
            transition={{ duration: reduce ? 0.2 : 2.8, ease: [0.72, 0, 0.28, 1] }}
...
function TanjoreDoorPanel({ t, side }: { t: TemplateTheme; side: "left" | "right" }) {
  return (
    <div
      className="relative h-full w-full overflow-hidden border bg-[#32100E]"
      style={{
        borderColor: `${t.gold}80`,
        background: `radial-gradient(ellipse at ${side === "left" ? "72%" : "28%"} 38%, #B66A5D44 0%, transparent 38%), repeating-linear-gradient(90deg, #2A0B0F 0px, #5A1719 9px, #35100F 18px, #742A1E 28px, #2A0B0F 40px), linear-gradient(145deg, #7E241E, #2A0B0F)`,
        boxShadow: side === "left" ? "inset -24px 0 36px #09020488, inset 10px 0 24px #F0B45A22" : "inset 24px 0 36px #09020488, inset -10px 0 24px #F0B45A22",
      }}
    >
      <span className="pointer-events-none absolute inset-3 border" style={{ borderColor: `${t.gold}88`, boxShadow: `0 0 22px ${t.gold}18 inset` }} />
      <span className="pointer-events-none absolute inset-6 border" style={{ borderColor: `${t.gold}3d` }} />
      <div className="absolute inset-x-7 top-8 bottom-10 grid grid-rows-4 gap-3">
        {Array.from({ length: 4 }, (_, i) => (
          <div key={i} className="relative overflow-hidden rounded-[4px] border shadow-[0_8px_14px_#16030666_inset,0_4px_12px_#16030666]" style={{ borderColor: `${t.gold}70`, background: "linear-gradient(145deg, #7B291E88, #2A0B0F88 62%, #8A3B2466)" }}>
            <span className="pointer-events-none absolute inset-1 border" style={{ borderColor: `${t.gold}35` }} />
            <TanjoreCarving t={t} variant={i} />
          </div>
        ))}
      </div>
      <span className={`absolute top-1/2 ${side === "left" ? "right-3" : "left-3"} h-12 w-2 -translate-y-1/2 rounded-full`} style={{ background: `linear-gradient(180deg, #F0D28A, ${t.gold}, #8C5A2B)`, boxShadow: `0 0 14px ${t.gold}88` }} />
      <span className={`absolute top-1/2 ${side === "left" ? "right-1.5" : "left-1.5"} h-5 w-5 -translate-y-1/2 rounded-full border`} style={{ borderColor: "#F0D28A", background: "#8C5A2B", boxShadow: `0 0 10px ${t.gold}88` }} />
      <span className="pointer-events-none absolute inset-x-0 bottom-4 text-center font-sans text-[7px] uppercase tracking-[0.24em] text-[#E8D39A77]">Tanjore Gold</span>
    </div>
  );
}

function TanjoreCarving({ t, variant }: { t: TemplateTheme; variant: number }) {
  if (variant === 0) {
    return (
      <svg viewBox="0 0 100 70" className="h-full w-full p-2" fill="none" aria-hidden="true">
        <path d="M18 57V31c0-11 14-20 32-20s32 9 32 20v26" stroke={t.gold} strokeOpacity="0.8" strokeWidth="1.7" />
        <path d="M27 57V33c0-7 10-13 23-13s23 6 23 13v24" stroke="#F0D28A" strokeOpacity="0.55" />
        <path d="M50 24v27M39 35c6-5 16-5 22 0M37 51c8-5 18-5 26 0" stroke={t.gold} strokeOpacity="0.7" strokeWidth="1.2" />
        <circle cx="50" cy="15" r="4" fill={t.gold} fillOpacity="0.35" stroke={t.gold} />
      </svg>
    );
  }
  if (variant === 1) {
    return (
      <svg viewBox="0 0 100 70" className="h-full w-full p-2" fill="none" aria-hidden="true">
        <path d="M50 58c-3-8-16-10-19-20-2-7 5-12 11-8l8 7 8-7c6-4 13 1 11 8-3 10-16 12-19 20Z" fill="#C9A34E33" stroke="#F0D28A" strokeWidth="1.4" />
        <path d="M50 8v11M38 15c4 4 8 5 12 4M62 15c-4 4-8 5-12 4M25 49c8-2 14 0 19 6M75 49c-8-2-14 0-19 6" stroke={t.gold} strokeOpacity="0.75" strokeWidth="1.1" strokeLinecap="round" />
        <circle cx="50" cy="8" r="2" fill="#F0D28A" />
      </svg>
    );
  }
  if (variant === 2) {
    return (
      <svg viewBox="0 0 100 70" className="h-full w-full p-2" fill="none" aria-hidden="true">
        <path d="M16 53c12-22 23-29 34-29s22 7 34 29M22 53c7-15 16-22 28-22s21 7 28 22" stroke={t.gold} strokeOpacity="0.8" strokeWidth="1.4" />
        <path d="M50 13 56 25 69 27 59 35 61 48 50 41 39 48 41 35 31 27 44 25Z" fill="#D5A64A44" stroke="#F0D28A" strokeWidth="1.2" />
        <circle cx="50" cy="29" r="4" fill={t.accent} stroke="#F0D28A" />
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 100 70" className="h-full w-full p-2" fill="none" aria-hidden="true">
      <path d="M30 54h40M35 54c0-8 7-12 15-12s15 4 15 12M40 42h20" stroke="#F0D28A" strokeWidth="1.5" />
      <path d="M50 42V19M44 27c4 3 8 3 12 0M42 20c5 3 11 3 16 0" stroke={t.gold} strokeOpacity="0.85" strokeWidth="1.2" strokeLinecap="round" />
      <path d="M50 13c-5 3-4 8 0 10 4-2 5-7 0-10Z" fill="#F7E6AF" stroke="#C9A34E" />
    </svg>
  );
}
...
function SealOpening({ t, template, data, onComplete }: { t: TemplateTheme; template: WeddingTemplate; data?: InvitationData; onComplete: () => void }) {
  ...
  const scratchStart = useRef<{ x: number; y: number } | null>(null);
  const reduce = useReducedMotion();
  ...
  const beginScratch = (event: ReactPointerEvent<HTMLDivElement>) => {
    if (stage !== "idle") return;
    scratchStart.current = { x: event.clientX, y: event.clientY };
  };

  const trackScratch = (event: ReactPointerEvent<HTMLDivElement>) => {
    if (stage !== "idle" || !scratchStart.current) return;
    const distance = Math.hypot(event.clientX - scratchStart.current.x, event.clientY - scratchStart.current.y);
    const progress = Math.min(100, Math.round((distance / 180) * 100));
    setScratchProgress(progress);
    if (progress >= 100) breakSeal();
  };

  const finishScratch = () => {
    scratchStart.current = null;
    if (stage === "idle") setScratchProgress(0);
  };
    ...
    >
      ...
      {stage === "idle" && (
        <motion.button
          ...
          onClick={breakSeal}
          whileTap={{ scale: 0.97 }}
          className="absolute bottom-7 left-1/2 z-50 inline-flex min-h-12 w-[calc(100vw-2rem)] max-w-[380px] -translate-x-1/2 items-center justify-center gap-2 overflow-hidden rounded-xl border px-5 py-3.5 font-sans text-[10px] font-medium uppercase tracking-[0.16em] text-[#FFF3D1] shadow-xl transition-transform hover:scale-[1.03] sm:bottom-10 sm:w-auto sm:max-w-none sm:px-9 sm:text-[11px] sm:tracking-luxe"
        ...
        >
          ...
          <Sparkles className="h-4 w-4" style={{ color: "#E8D39A" }} strokeWidth={1.5} />
          <motion.span className="absolute bottom-0 left-0 h-0.5 bg-[#E8D39A]" animate={{ width: `${scratchProgress}%` }} />
        </motion.button>
      )}
      {stage === "idle" && (
        <p className="pointer-events-none absolute bottom-[4.65rem] left-1/2 z-40 w-[calc(100vw-2rem)] -translate-x-1/2 text-center font-sans text-[9px] uppercase tracking-[0.14em] text-[#E8D39A99] sm:bottom-24 sm:w-auto sm:text-[10px]">
          {scratchProgress > 0 ? "Keep moving to reveal" : "Tap the seal or gently scratch across the invitation"}
        </p>
      )}
    </div>
  );
