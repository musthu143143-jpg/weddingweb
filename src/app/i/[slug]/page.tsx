...
export default async function PublicInvitationPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const invitationView = <InvitationDemo template={template} data={data} publicView invitationId={invitation.id} />;
  return template.opening ? (
    <OpeningGate template={template} data={data}>{invitationView}</OpeningGate>
  ) : invitationView;
}
